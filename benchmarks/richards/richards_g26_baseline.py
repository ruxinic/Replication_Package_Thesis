...
def run_benchmark(loops):
    # Runs all iterations sequentially on a single core
    richard = Richards()
    t0 = perf.perf_counter()
    for _ in range(loops):
        richard.run(30)  # Scale up sequential iterations for a larger footprint
    return perf.perf_counter() - t0

if __name__ == "__main__":
    runner = perf.Runner()
    runner.metadata['description'] = "The Richards benchmark - Sequential Baseline"
    
    cmd = runner.parse_args()
    if cmd.loops is None:
        cmd.loops = 5  
        
    runner.bench_time_func('richards_baseline', run_benchmark)