...
class Packet(object):
    ...
    def append_to(self, lst):
        ...
        # G3: if the destination list doesn't have a next item, link it directly and exit early
        if lst.link is None:
            lst.link = self
            return lst
...
class WorkTask(Task):
    ...
    def fn(self, pkt, r):
            ...
            # G3: unroll the 4-iteration loop completely to remove loop overhead
            w_count = w.count

            # it 0
            w_count += 1
            if w_count > 26: w_count = 1
            pkt.data[0] = A + w_count - 1

            # it 1
            w_count += 1
            if w_count > 26: w_count = 1
            pkt.data[1] = A + w_count - 1

            # it 2
            w_count += 1
            if w_count > 26: w_count = 1
            pkt.data[2] = A + w_count - 1

            # it 3
            w_count += 1
            if w_count > 26: w_count = 1
            pkt.data[3] = A + w_count - 1

            w.count = w_count
            return self.qpkt(pkt)

def schedule():
    # G3: cache loop-invariant states and methods into local variables
    ...
    while t is not None:
        ...
        # replace the method call with direct attribute lookup
