...
class HandlerTaskRec(TaskRec):
    ...
    # G19: inlined wrapper functions into HandlerTask.fn to avoid lookup delays
...
class TaskState(object):
    ...
    # G19: getter and setter methods are left intact for initialization steps,
    # but the core performance loops bypass them using inlined properties
...
class Task(TaskState):
    ...
    def __init__(self, i, p, w, initialState, r):
        ...
        # G19: inlined methods into direct property queries during instantiation
        self.packet_pending = initialState.packet_pending
        self.task_waiting = initialState.task_waiting
        self.task_holding = initialState.task_holding
    ...
    def runTask(self):
        # G19: inlined `isWaitingWithPacket` state method lookup
        if self.packet_pending and self.task_waiting and not self.task_holding:
            msg = self.input
            self.input = msg.link
            if self.input is None:
                # G19: inlined `running()` setter logic to eliminate frame overhead
                self.packet_pending = False
                self.task_waiting = False
                self.task_holding = False
            else:
                # G19: inlined `packetPending()` setter logic to eliminate frame overhead
                self.packet_pending = True
                self.task_waiting = False
                self.task_holding = False
        else:
...
class HandlerTask(Task):
    ...
    def fn(self, pkt, r):
        ...
        if pkt is not None:
            if pkt.kind == K_WORK:
                # G19: inlined wrapper lookup `h.workInAdd(pkt)`
                h.work_in = pkt.append_to(h.work_in)
            else:
                # G19: inlined wrapper lookup `h.deviceInAdd(pkt)`
                h.device_in = pkt.append_to(h.device_in)
        work = h.work_in
        if work is None:
            return self.waitTask()
        count = work.datum
...
def schedule():
    ...
    while t is not None:
        ...
        # G19: inlined `t.isTaskHoldingOrWaiting()`
...
if __name__ == "__main__":
    runner = perf.Runner()
    runner.metadata['description'] = "The Richards benchmark with G19 Inlined Functions"
