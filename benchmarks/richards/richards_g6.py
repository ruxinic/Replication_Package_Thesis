...
# G6: deleted global debugging variables 'tracing' and 'layout', and the trace() function entirely
...
class Task(TaskState):
    ...
    def runTask(self):
        # G6: streamline logic to avoid recomputing or over-checking self.input states
        msg = self.input
        if msg is not None:
            self.input = msg.link
            if self.input is None:
...
class DeviceTask(Task):
    ...
    def fn(self, pkt, r):
        ...
        if pkt is None:
            ...
            d.pending = pkt
            # G6: removed 'if tracing:' debugging code path
            return self.hold()
...
def schedule():
    ...
    while t is not None:
        # G6: removed 'if tracing:' debugging checks
...
class Richards(object):
    ...
    def run(self, iterations):
        for i in range(iterations):
            ...
            # G6: deleted the variable assignment `wkq = None`
