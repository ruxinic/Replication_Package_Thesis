...
class IdleTask(Task):
    ...
    def fn(self, pkt, r):
        ...
        if i.count == 0:
            return self.hold()
        cntr_expression = i.control // 2
        if i.control & 1 == 0:
            i.control = cntr_expression
            return self.release(I_DEVA)
        else:
            i.control = cntr_expression ^ 0xd008
...
class WorkTask(Task):
    ...
    def fn(self, pkt, r):
        ...
        pkt.ident = dest
        pkt.datum = 0
    # G1: cache the object attribute to a local variable before the loop
        w_count = w.count  
        for i in BUFSIZE_RANGE:
            w_count += 1
            if w_count > 26:
                w_count = 1
            pkt.data[i] = A + w_count - 1

        w.count = w_count # Write back once
        return self.qpkt(pkt)

def schedule():
    local_tracing = tracing # G1: store global variable locally
