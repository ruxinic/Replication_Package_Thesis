...
def richard_worker(iterations_chunk):
    # worker function tasked to run a subset of iterations on an isolated core
    richard = Richards()
    return richard.run(iterations_chunk)

def run_parallel_richards(loops, num_cores, total_iterations):
    iterations_per_core = total_iterations // num_cores
    
    t0 = perf.perf_counter()
    for _ in range(loops):
        # G26: allocating all computational cores simultaneously
        with ProcessPoolExecutor(max_workers=num_cores) as executor:
            futures = [
                executor.submit(richard_worker, iterations_per_core)
                for _ in range(num_cores)
            ]
            # ensure every worker finishes its core assignment
            for future in futures:
                success = future.result()
                if not success:
                    raise RuntimeError("Benchmark integrity check failed.")
                    
    return perf.perf_counter() - t0

if __name__ == "__main__":
    runner = perf.Runner()
    runner.metadata['description'] = "The Richards benchmark - Parallel Saturation (G26)"
    ...
    runner.metadata['cpu_cores_allocated'] = num_cores
    
    # pass total benchmark execution targets down to the parallel worker harness
    runner.bench_time_func(
        'richards_g26', 
        run_parallel_richards, 
        num_cores, 
        30
    )