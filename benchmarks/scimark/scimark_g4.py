...
class Array2D(object):
    ...
    def _idx(self, x, y):
        # G4: if x >= width is True, Python short-circuits and skips 
        # checking y, or the slower negative checks entirely
...
class ArrayList(Array2D):
    ...
    def __getitem__(self, idx):
        # G4: check for the highly frequent row-index (int) type path first
        if type(idx) is int:
            return self.data[idx]
        return self.data[idx[1]][idx[0]]

    def __setitem__(self, idx, val):
            # G4: aligned with the __getitem__ optimization path
            if type(idx) is int:
                self.data[idx] = val
            else:
