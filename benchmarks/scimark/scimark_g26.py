...
def sor_worker(loops_chunk, n, cycles, Array):
    for _ in range(loops_chunk):
        G = Array(n, n)
        SOR_execute(1.25, G, cycles, Array)

def parallel_bench_SOR(loops, n, cycles, Array):
    num_cores = os.cpu_count() or 1
    loops_per_core = max(1, loops // num_cores)
    
    t0 = perf.perf_counter()
    # G26: allocate all computational cores simultaneously
    with ProcessPoolExecutor(max_workers=num_cores) as executor:
        futures = [
            executor.submit(sor_worker, loops_per_core, n, cycles, Array)
...
def sparse_worker(cycles_chunk, N, y, val, row, col, x):
    SparseCompRow_matmult(N, y, val, row, col, x, cycles_chunk)

def parallel_bench_SparseMatMult(cycles, N, nz):
    num_cores = os.cpu_count() or 1
    cycles_per_core = max(1, cycles // num_cores)
    ...
    for r in range(N):
        ...
        row[r + 1] = rowr + nr
        if step < 1: step = 1
        for i in range(nr): col[rowr + i] = i * step

    t0 = perf.perf_counter()
    with ProcessPoolExecutor(max_workers=num_cores) as executor:
        futures = [
            executor.submit(sparse_worker, cycles_per_core, N, y, val, row, col, x)
            for _ in range(num_cores)
        ]
        for future in futures:
            future.result()
...
def mc_worker(loops_chunk, Num_samples):
    for _ in range(loops_chunk):
        MonteCarlo(Num_samples)

def parallel_bench_MonteCarlo(loops, Num_samples):
    num_cores = os.cpu_count() or 1
    loops_per_core = max(1, loops // num_cores)
    ...
    with ProcessPoolExecutor(max_workers=num_cores) as executor:
        futures = [
            executor.submit(mc_worker, loops_per_core, Num_samples)
            for _ in range(num_cores)
        ]
        for future in futures:
            future.result()
...
def lu_worker(cycles_chunk, N, A, lu, pivot):
    for _ in range(cycles_chunk):
        LU(lu, A, pivot)

def parallel_bench_LU(cycles, N):
    num_cores = os.cpu_count() or 1
    cycles_per_core = max(1, cycles // num_cores)
    ...
    with ProcessPoolExecutor(max_workers=num_cores) as executor:
        futures = [
            executor.submit(lu_worker, cycles_per_core, N, A, lu, pivot)
            for _ in range(num_cores)
        ]
        for future in futures:
            future.result()
...
def fft_worker(loops_chunk, N, cycles, init_vec):
    twoN = 2 * N
    for _ in range(loops_chunk):
        x = array('d')
        x[:] = init_vec[:]
...
def parallel_bench_FFT(loops, N, cycles):
    num_cores = os.cpu_count() or 1
    loops_per_core = max(1, loops // num_cores)
    twoN = 2 * N
    ...
    with ProcessPoolExecutor(max_workers=num_cores) as executor:
        futures = [
            executor.submit(fft_worker, loops_per_core, N, cycles, init_vec)
            for _ in range(num_cores)
        ]
        for future in futures:
            future.result()
...
BENCHMARKS = {
    'sor': (parallel_bench_SOR, 100, 10, Array2D),
    'sparse_mat_mult': (parallel_bench_SparseMatMult, 50000, 1000000),
    'monte_carlo': (parallel_bench_MonteCarlo, 1000000),
    'lu': (parallel_bench_LU, 256),
    'fft': (parallel_bench_FFT, 16384, 7),
}
...
if __name__ == "__main__":
    ...
    for bench in benchmarks:
        name = 'scimark_%s_g26' % bench
        args_payload = BENCHMARKS[bench]
        runner.bench_time_func(name, *args_payload)