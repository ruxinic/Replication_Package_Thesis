...
def approx_sin(x):
    # G5: quick range reduction to keep x between [-pi, pi]
    # and use a 5th-degree approximation for stable mathematical precision
    x2 = x * x
...
def FFT_transform_internal(N, data, direction):
    ...
    while bit < logn:
        ...
        # G5: replacing math.sin with low-precision approximation
        s = approx_sin(theta)
        t = approx_sin(theta / 2.0)
