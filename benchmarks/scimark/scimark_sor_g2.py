...
def SOR_execute(omega, G, cycles, Array):
    # G2: stop iterating over portions or the entirety of a collection 
    # if it has already reached its sorted/converged state.
    tolerance = 1e-6
    
    for p in range(cycles):
        converged = True  # track if any major updates happen in this pass
        ...
        for y in range(1, G.height - 1):
            for x in range(1, G.width - 1):
                ...
                # compute new value
                new_val = (omega * 0.25 * (G[x, y - 1] + G[x, y + 1] + G[x - 1, y]
                                           + G[x + 1, y])
                           + (1.0 - omega) * old_val)
                
                # G2: check if the collection element is already in its final/ordered spot
                if abs(new_val - old_val) > tolerance:
                    converged = False
                
                G[x, y] = new_val
                
        # if no values shifted noticeably, the matrix is "sorted" (fully converged)
        # avoid redundant cycles entirely
        if converged:
            break
