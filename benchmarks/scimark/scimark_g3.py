...
class Array2D(object):
    def __init__(self, w, h, data=None):
        self.width = w
        self.height = h
        # G3: end condition stored in a variable
        size = w * h 
    ...
    def _idx(self, x, y):
        # G3: cache attributes used in boundary checks
    ...
    def setup(self, data):
        # G3: store loop end conditions in local variables
...
class Random(object):
    ...
    def initialize(self, seed):
        ...
        m2 = self.m2
        m2_half = m2 / 2
        # G3: loop end condition is constant, but logic inside is optimized
        for iloop in range(17):
            jseed = j0 * k0
            j1 = (jseed / m2 + j0 * k1 + j1 * k0) % m2_half
            j0 = jseed % m2
...
def SOR_execute(omega, G, cycles, Array):
    ...
    # G3: optimize the inner loop calculations
    om_025 = omega * 0.25
    one_minus_om = 1.0 - omega

    for p in range(cycles):
        for y in range(1, h_limit):
            for x in range(1, w_limit):
                # G3: minimize overhead by using the pre-calculated limits
                G[x, y] = (om_025 * (G[x, y - 1] + G[x, y + 1] + G[x - 1, y] + G[x + 1, y])
                           + one_minus_om * G[x, y]) 
...
def MonteCarlo(Num_samples):
    rnd = Random(113)
    # G3: cache method lookup outside the loop to reduce per-iteration overhead
    next_double = rnd.nextDouble
    under_curve = 0
...
def LU_factor(A, pivot):
    ...
    for j in range(minMN):
        ...
        # G3: check for failure condition early 
        if A[jp][j] == 0:
        ...
        if j < minMN - 1:
            # g3
            next_j = j + 1 
            for ii in range(next_j, M):
                for jj in range(next_j, N):
...
def FFT_transform_internal(N, data, direction):
    ...
    dual = 1
    # G3: use the range object without manual increment
    for bit in range(logn):
        ...
        # first b-loop handles the first element of each block (w_real=1, w_imag=0)
        for b in range(0, n, two_dual): 
            i = 2 * b
            j = i + two_dual 
            wd_real, wd_imag = data[j], data[j + 1]
            data[j], data[j + 1] = data[i] - wd_real, data[i + 1] - wd_imag
            data[i], data[i + 1] = data[i] + wd_real, data[i + 1] + wd_imag
            
        # initialize twiddle factors for the 'a' loop
        w_real, w_imag = 1.0, 0.0
        
        for a in range(1, dual):
            # G3: logic remains optimized, but variables are now properly initialized
            ...
            for b in range(0, n, two_dual):
                i = 2 * (b + a)
                j = i + two_dual
                z1_real, z1_imag = data[j], data[j + 1]
...
def FFT_inverse(N, data):
    ...
    n_val = N / 2
    norm = 1 / float(n_val)
    limit = N # G3: end condition variable
