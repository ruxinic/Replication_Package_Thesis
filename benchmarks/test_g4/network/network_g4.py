...
def process_stream_baseline(stream):
    ...
    # G4: flattened short-circuit check.
        # if 'packet is not None' or 'payload_len > 0' fails, the heavy math is skipped
        if (packet is not None and 
            packet["payload_len"] > 0 and 
            packet["ip"] != "0.0.0.0" and 
            (packet["payload_len"] ** 7) % 9999991 > 0):
            allowed_count += 1
