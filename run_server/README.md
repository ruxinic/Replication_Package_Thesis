...
- **Standard version (GL1 server)** - used for the majority of the experiment. Runs each script directly through EnergiBridge with no additional flags.
- **GPU version (GLG1 server)** - used specifically for G23 and G25, which required execution on the secondary, GPU-equipped server. This version adds the `--gpu` flag to EnergiBridge to capture GPU energy metrics, and pipes the script's output through `head` to limit console clutter from GPU-related logging.
